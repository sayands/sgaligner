...
from ext.scenegraphfusion.utils import util, util_label
from ext.scenegraphfusion.utils.util_search import SAMPLE_METHODS,find_neighbors
from ext.scenegraphfusion.utils import dataLoaderScanNet
debug = True
debug = False
...
def Parser(add_help=True):
    ...
    parser.add_argument('--label_type', type=str,default='3RScan', 
...
def process(pth_scan, scan_id, label_type, verbose=False) -> list:
    verbose=False
    edge2idx = name2idx(os.path.join(define.SCANNET_FILES_PATH, 'relationships.txt'))
    ...
    pth_pd  = os.path.join(pth_scan, scan_id, args.scan_name)
    pth_ply = os.path.join(pth_scan, scan_id, scan_id+define.SCANNET_PLY_SUBFIX)
    pth_agg = os.path.join(pth_scan, scan_id, scan_id+define.SCANNET_AGGRE_SUBFIX)
    pth_seg = os.path.join(pth_scan, scan_id, scan_id+define.SCANNET_SEG_SUBFIX)
    ...
    pred_rel_filename = os.path.join(pth_scan, scan_id, 'predictions.json')

    predictions = common.load_json(pred_rel_filename)[scan_id]
    ...
    for edge in edges.keys():
        ...
        if int(sub) is None or int(obj) is None: 
            print('hi')
        
        if best_edge_name not in ['none', name_same_segment] and best_edge_name is not None:
            relationship_data.append([int(sub), int(obj), int(edge2idx[best_edge_name]), best_edge_name])
    ...
    for gt_segment, pd_segments in sorted(gt_segments_2_pd_segments.items()):
        ...
        for pd_segment in pd_segments:
            ...
            pd_segment_ids.append(pd_segment)     
        print('')
    
    pd_segment_ids = np.unique(pd_segment_ids)
    rel_data = []
    for rel in relationship_data:
        if rel[0] in pd_segment_ids and rel[1] in pd_segment_ids and rel[3] not in [name_same_segment, 'none'] :
            sub = get_key_by_value(gt_segments_2_pd_segments, rel[0])
            ob = get_key_by_value(gt_segments_2_pd_segments, rel[1])

            if sub == ob: continue

            if sub is None or ob is None:
                print(sub, ob)
                print(gt_segments_2_pd_segments)
                assert False

            row_exists = False
            if len(rel_data) > 0:
                row_exists = np.any(np.all(np.array(rel_data) == np.array([sub, ob, rel[2], rel[3]]), axis=1))
            if not row_exists: rel_data.append([sub, ob, rel[2], rel[3]])
...
if __name__ == '__main__':
    ...
    relationships, segs_neighbors = process(define.SCANNET_SCENES_PATH, scan_id,label_type = args.label_type, verbose = args.verbose)
