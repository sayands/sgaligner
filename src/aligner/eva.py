...
class EVA(nn.Module):
    def __init__(self, rel_dim, attr_dim, n_units=[3, 128, 128], emb_dim = 100, pt_out_dim = 256, dropout = 0.0, attn_dropout = 0.0, instance_norm = False):
    ...
    def forward(self, data_dict):
        ...
        embs['gcn'] = self.structure_embedding(structure_embed)
            
        embs['point'] = self.object_encoder(tot_object_points)
        embs['point'] = self.object_embedding(embs['point'])

        embs['rel'] = self.meta_embedding_rel(tot_bow_vec_object_edge_feats)
        embs['attr'] = self.meta_embedding_attr(tot_bow_vec_object_attr_feats)

        all_embs = []
        for module in embs.keys():
